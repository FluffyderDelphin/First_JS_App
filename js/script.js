...
let pokemon_reposetory = (function () {
  ...
  function addListItem(pokemon) {
    ...
    let button =
      $(`<button type="button" class="btn btn-warning pokemonButton" data-toggle="modal" data-target="#pokemonModal">
  ...
  function showDetails(pokemon) {
    loadDetails(pokemon).then(() => {
      ...
      modalTitle.append(
        $(
          `<h2 class="modal-pokemon-name">#${pokemon.entry} ${capitalizeName(
            pokemon.name
          )}</h2>`
        )
      );
      ...
      imageContainer.append(
        $(
          `<img class="pokemon-image img-fluid" alt="image of ${pokemon.name} from the Front" src="${pokemon.imageFront}"></img>`
        )
      );
      imageContainer.append(
        $(
          `<img class="pokemon-image img-fluid" alt="image of ${pokemon.name} from the Back" src="${pokemon.imageBack}"></img>`
        )
      );
      ...
      detail_list.append(
        $(
          `<p class="list-group-item text-center border-0 sr-only-focusable">Height : ${
            pokemon.height / 10
          } m</p>`
        )
      );
      detail_list.append(
        $(
          `<p class="list-group-item text-center border-0 sr-only-focusable">Weight : ${
            pokemon.weight / 10
          } kg</p>`
        )
      );
  ...
  function prepareType(type) {
    ...
    type.forEach((e) => {
      let type_entry = $(
        `<span class="pokemon-type">${e.toUpperCase()} </span>`
      );
...
pokemon_reposetory.loadlist().then(() => {
